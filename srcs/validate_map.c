...
/*   Updated: 2025/03/11 20:13:41 by joaomart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	map_height(t_game *info, char *f_name)
{
	...
	close(fd);
	if (n_lines == 0)
		ft_error("Altura do mapa invalida!");
...
void	validate_map_walls(t_game *info)
{
	...
	while (x <= end_x || y <= end_y)
	{
		if (info->map_info.grid[y][0] != '1' || info->map_info.grid[0][x] != '1'
			...
			ft_error_map(info, "Parede do mapa incompleta!");
		else if (y < end_y)
			y++;
		else if (x < end_x)
			x++;
