...
/*   Updated: 2025/03/11 20:13:03 by joaomart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	n_of_elements(t_game *info)
{
	if (info->map_info.n_player != 1)
		ft_error_map(info, "Numero de jogadores errado!");
	if (info->map_info.n_exit != 1)
		ft_error_map(info, "Numero de saidas errado!");
	if (info->map_info.n_collects == 0)
		ft_error_map(info, "Numero de coletaveis insuficiente!");
}
	...
	while (x <= end_x || y <= end_y)
	{
		...
		while (y <= end_y)
		{
			...
			if (unknown_element(info, x, y))
				ft_error_map(info, "Elementos invalidos!");
			y++;
		}
		x++;
	}
