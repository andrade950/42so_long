...
/*   Updated: 2025/03/10 15:27:00 by joaomart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ac > 2)
		ft_error("Argumentos a mais!");
	else if (ac < 2)
		ft_error("Argumentos a menos!");
}
...
void	check_filename(char *f_name)
{
	...
	while (f_name[i])
	{
		if (f_name[i] == type[j])
		{
			...
			ft_error("Nome do ficheiro errado!");
	}
}
...
void	check_map_exists(char *f_name)
{
	...
	if (fd < 0)
		ft_error_fd(fd, "Ficheiro nao existe!");
}
