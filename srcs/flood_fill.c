...
/*   Updated: 2025/03/11 20:14:56 by joaomart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	validate_coor(t_game *info)
{
	if (info->map_info.player_x < 0
		...
		ft_error_map(info, "Coordenadas invalidas!");
}
...
char	**copy_map(t_game *info)
{
	...
	if (!map_copy)
		ft_error_map(info, "Falha ao copia mapa!");
...
{
	if (px < 0 || py < 0 || map_copy[py][px] == '1' || map_copy[py][px] == 'X'
		|| map_copy[py][px] == 'F')
		return ;
	if (map_copy[py][px] == '0' || map_copy[py][px] == 'P'
			|| map_copy[py][px] == 'C' || map_copy[py][px] == 'E')
		map_copy[py][px] = 'F';
...
void	elements_locked(char **map_copy, int i)
{
	...
	while (map_copy[i][j])
	{
		if (map_copy[i][j] == 'P' || map_copy[i][j] == 'C'
			|| map_copy[i][j] == 'E')
		{
			ft_error("Elementos do mapa fechados!");
			while (map_copy[i])
				free(map_copy[i++]);
			free(map_copy);
		}
